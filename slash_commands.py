...
def slash_coms(token, id:str):
    ...
    if token == ID_TOKEN:
        prompt = f"""Użytkownik podał ci właśnie klienta o client_id = {id} z tabeli clients,
                     która znajduje się w schemacie {DB_SCHEMA}. Użyj narzędzia "query_database()". Masz na celu znalezienie rekordu
                     z tym id, a następnie z kolumny 'client' pobierz dane. Dane te są zapisane w JSON object.
                     Jako odpowiedź wypisz tylko te dane z odpowiednimi opisami w taki sposób by były jak najczytelniejsze dla użytkownika.
